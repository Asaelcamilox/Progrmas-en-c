...
void llenarMatriz5(char *nombre, int matriz[3][3]) {
    printf("Nombre de la matriz: %s\n", nombre);
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            printf("Ingrese el valor para la posicion [%d][%d]: ", i, j);
...
void sumarMatriz2(int matriz1[3][3], int matriz2[3][3], int matrizr[3][3]) {
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            matrizr[i][j] = matriz1[i][j] + matriz2[i][j];
        }
...
void restaMatriz2(int matriz1[3][3], int matriz2[3][3], int matrizr[3][3]) {
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            matrizr[i][j] = matriz1[i][j] - matriz2[i][j];
        }
...
void escalarMatriz2(int matriz1[3][3], int escalar, int matrizr[3][3]) {
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            matrizr[i][j] = matriz1[i][j] * escalar;
        }
...
int determinanteSarrus2(int matriz[3][3]) {
    int suma_diagonal = 0, suma_inversa = 0;
    for (int i = 0; i < 3; i++) {
        int producto = 1;
        for (int j = 0; j < 3; j++) {
...
int main(){
    ...
    setColor("\033[0m");  
        ...
        case 64:{
            ...
            llenarMatriz5("Matriz A", matriz_a);
            llenarMatriz5("Matriz B", matriz_b);
        
            clearScreen();
            
            sumarMatriz2(matriz_a, matriz_b, matriz_suma);
            restaMatriz2(matriz_a, matriz_b, matriz_resta);
            escalarMatriz2(matriz_a, escalar, matriz_escalar);
            
            // Imprimir matrices con bordes laterales
            ...
            gotoxy(5, 10);
            int det = determinanteSarrus2(matriz_a);
            printf("Determinante de Matriz A (Sarrus): %d\n", det);
